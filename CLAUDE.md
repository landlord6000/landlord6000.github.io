...
НИКОГДА НЕ МЕНЯЙ АВТОРСКИЙ ТЕКСТ, НИ ПРИ КАКИХ ОБСТАЯТЕЛСЬТВАХ. МЕНЯ ТОЛЬКО ЕСЛИ ТЕБЯ ЯВНО ПОПРОСЯТ ВНЕСТИ КАКИЕ-ТО ИЗМЕНЕНЕИЯ.
...
1. **`<slug>.md`** — причёсанный черновик-конспект: тот же текст, разбитый заголовками по блокам (точки/дни), с пометками, где по тексту должны стоять фото — `[фото NN, MM: подпись]`, и где видео — `[видео NN: подпись]`. Промежуточный шаг для проверки текста и структуры человеком, без CSS/HTML.
...
travell/<slug>/videos/   — видео (если есть)
```
...
Если по тексту встречается пометка вида `видеоNN` (в черновике — `[видео NN: подпись]`), это отдельный файл `videos/NN.mp4`. Нумерация видео — своя, независимая от фото (файлы лежат в отдельной папке, поэтому `видео00` и `фото00` не конфликтуют). Постер (превью-кадр) не обязателен; если он есть — кладётся туда же с тем же именем: `videos/NN.jpg`, и указывается в атрибуте `poster`.
...
<body>
  ...
  <div class="wrap">
    ...
    <!-- Видео (широкий кадр 16:10) — подпись снизу опциональна -->
    <div class="video-wrap">
      <video controls preload="metadata" poster="./videos/00.jpg">
        <source src="./videos/00.mp4" type="video/mp4">
      </video>
    </div>
    <div class="caption">Подпись под видео</div>
...
- `<div class="video-wrap">` с `<video controls>` — для видео (широкий кадр 16:10, как одиночное фото). Источник `./videos/NN.mp4`, при наличии превью-кадра — атрибут `poster="./videos/NN.jpg"`. Подпись `<div class="caption">` под видео опциональна. В лайтбокс видео не открывается (там только фото).
